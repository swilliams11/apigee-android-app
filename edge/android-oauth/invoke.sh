...
#TODO update this to a sample proxy that will accept tokens generated for this client id.
targetservice="https://$org-$env.$api_domain/android-oauth"
#########################################################################################################
...
accesstoken_request="https://$org-$env.$api_domain/android-oauth/token"
...
echo -e "\n\nSending test request to $targetservice\n"
echo -e "curl -H \"Authorization: Bearer $access_token\" \"$targetservice\" -i"

targetresponse=`curl -H "Authorization: Bearer $access_token" "$targetservice" -i`

echo -e "\n\nResponse:\n"
echo -e "$targetresponse"
